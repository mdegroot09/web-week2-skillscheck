...
const filtered = myNumbers.filter(function(num){
	for(i = 0; i < myNumbers.length; i++){
		if (num > 100){
			return num;
		}
...
const total = myNumbers.reduce(function(prev, num){
	for (i = 0; i < myNumbers.length; i++){
		return prev + num
	}
}, 0)
...
const forTheLoveOfGeorge = notGeorge.map(function(){
	return "George"
})
...
const totallyAwesome = people.reduce(function(prev, person){
	for (i = 0; i < people.length; i++){
		return prev + person.awesomeLevel
	}
}, 0)
